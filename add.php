<?php 
    ...
    if (isset($_REQUEST['btn_insert'])) {
        try {
            ...
            $path = "fileupload/" . $image_file; // set upload folder path
            ...
            if (empty($name)) {
                ...
                if (!file_exists($path)) { // check file not exist in your upload folder path
                    if ($size < 5000000) { // check file size 5MB
                        move_uploaded_file($temp, 'fileupload/'.$image_file); // move upload file temperory directory to your upload folder
                    } else {
